...
const BLANK_WAYPOINT = {
  basePrice: 500,
  dateFrom: '2023-07-17T17:17:17.375Z',
  dateTo: '2023-07-17T17:17:17.375Z',
...
function createEditFormTemplate(isEditForm, oneWaypoint, offers, destinations) {
  const itemDest = getItemById(destinations, oneWaypoint.destination);
    ...
    <header class="event__header">
      ...
      <div class="event__field-group  event__field-group--destination">
        <label class="event__label  event__type-output" for="event-destination-${oneWaypoint.id}">
          ${capitalizeType(oneWaypoint.type)}
        </label>
      ...
      <div class="event__field-group  event__field-group--time">
        ...
        <input class="event__input  event__input--time" id="event-start-time-${oneWaypoint.id}" type="text" name="event-start-time" value="${getDateWithYears(oneWaypoint.dateFrom)}" ${(oneWaypoint.isDisabled) ? 'disabled' : ''}>
        &mdash;
        <label class="visually-hidden" for="event-end-time-${oneWaypoint.id}">To</label>
        <input class="event__input  event__input--time" id="event-end-time-${oneWaypoint.id}" type="text" name="event-end-time" value="${getDateWithYears(oneWaypoint.dateFrom)}" ${(oneWaypoint.isDisabled) ? 'disabled' : ''}>
      </div>
  ...
  #setFromDatePicker() {
    this.#fromDatepicker = flatpickr(
      this.element.querySelector(`#event-start-time-${this._state.id}`),
      {
        ...
        defaultDate: getDateWithYears(this._state.dateFrom),
  ...
  #setToDatePicker() {
    this.#toDatepicker = flatpickr(
      this.element.querySelector(`#event-end-time-${this._state.id}`),
      {
        enableTime: true,
        dateFormat: 'd/m/y H:i',
        defaultDate: getDateWithYears(this._state.dateTo),
        minDate: getDateWithYears(this._state.dateFrom),
