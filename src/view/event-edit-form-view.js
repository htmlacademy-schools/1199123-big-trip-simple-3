...
import { convertToBasicDate } from '../utils/date.js';
import { pointTypes } from '../mock/mock-const.js';
  ...
  return (
    ...
    <header class="event__header">
      ...
      <div class="event__field-group  event__field-group--time">
        ...
        <input class="event__input event__input--time" id="event-start-time-${tripPoint.id}" type="text" name="event-start-time" value="${convertToBasicDate(tripPoint.dateFrom)}">
        &mdash;
        <label class="visually-hidden" for="event-end-time-${tripPoint.id}">To</label>
        <input class="event__input event__input--time" id="event-end-time-${tripPoint.id}" type="text" name="event-end-time" value="${convertToBasicDate(tripPoint.dateTo)}">
      </div>
...
export default class EditFormView extends AbstractStatefulView {
  ...
  constructor({tripPoint = BLANK_TRIPPOINT, destinations, offers, onFormSubmit = () => (0), isEditForm = true}) {
    super();
    this._setState(EditFormView.parseTripPointToState(tripPoint));
  ...
  reset(tripPoint) {
    this.updateElement(
      EditFormView.parseTripPointToState(tripPoint),
    );
  }
